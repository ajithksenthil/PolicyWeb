...
def generate_policy_cards(user_needs):
    ...
    # use vector store to check if there are duplicates
    policy_cards = []
    ...
    # store the new policy cards in the vector store
    return policy_cards
...
if conversations:
    ...
    if st.session_state.current_concern_index < len(st.session_state.concerns):
        ...
        # save structured_needs (user needs) to the relational data base
